// lib/mapStrapiPressRelease.ts

// ---------------- Text & Content Types ----------------
export interface StrapiTextNode {
  text: string;
}

export interface StrapiContentBlock {
  type: string;
  children: StrapiTextNode[];
}

// ---------------- Media & Attachment Types ----------------
export interface StrapiThumbnail {
  url: string;
  alternativeText?: string | null;
}

export interface StrapiAttachmentFile {
  id: number;
  name: string;
  url: string;
  mime?: string;
  size?: number;
}

export interface StrapiAttachment {
  id: number;
  label: string;
  file?: StrapiAttachmentFile;
}

// ---------------- Tag ----------------
export interface StrapiTag {
  id: number;
  name: string;
  slug: string;
}

// ---------------- Main Press Release ----------------
export interface StrapiPressRelease {
  id: number;
  title: string;
  slug: string;
  date: string;
  content: StrapiContentBlock[];
  thumbnail?: StrapiThumbnail;
  attachments?: StrapiAttachment[];
  tags?: StrapiTag[];
  is_featured?: boolean;
  priority?: number;
  pinned_until?: string | null;
}

// ---------------- Raw API Types ----------------
interface RawAttachment {
  id: number;
  label: string;
  file?: {
    id: number;
    name: string;
    url: string;
    mime?: string;
    size?: number;
  };
}

interface RawTag {
  id: number;
  name: string;
  slug: string;
}

interface FlatUpdateResponse {
  id: number;
  title: string;
  slug: string;
  date: string;
  content: StrapiContentBlock[];
  thumbnail?: StrapiThumbnail;
  attachments?: RawAttachment[];
  tags?: RawTag[];
  category?: string;
  is_featured?: boolean;
  priority?: number;
  pinned_until?: string | null;
}

interface NestedUpdateResponse {
  id: number;
  attributes: {
    title: string;
    slug: string;
    date: string;
    content: StrapiContentBlock[];
    thumbnail?: StrapiThumbnail;
    attachments?: RawAttachment[];
    tags?: RawTag[];
    category?: string;
    is_featured?: boolean;
    priority?: number;
    pinned_until?: string | null;
  };
}

export type StrapiResponseItem = FlatUpdateResponse | NestedUpdateResponse;

// ---------------- Mapper ----------------
export function mapStrapiPressRelease(item: StrapiResponseItem): StrapiPressRelease {
  const attrs = "attributes" in item ? item.attributes : item;

  // ✅ Only map press releases
  if (attrs.category !== "Press Release") {
    throw new Error("Not a press release");
  }

  // Map attachments
  const attachments: StrapiAttachment[] | undefined = attrs.attachments?.map(
    (att: RawAttachment) => ({
      id: att.id,
      label: att.label || "",
      file: att.file
        ? {
            id: att.file.id,
            name: att.file.name,
            url: att.file.url.startsWith("http")
              ? att.file.url
              : `${process.env.NEXT_PUBLIC_CMS_URL}${att.file.url}`,
            mime: att.file.mime,
            size: att.file.size,
          }
        : undefined,
    })
  );

  // Map thumbnail
  const thumbnail: StrapiThumbnail | undefined = attrs.thumbnail
    ? {
        url: attrs.thumbnail.url.startsWith("http")
          ? attrs.thumbnail.url
          : `${process.env.NEXT_PUBLIC_CMS_URL}${attrs.thumbnail.url}`,
        alternativeText: attrs.thumbnail.alternativeText || null,
      }
    : undefined;

  // Map tags (array)
  const tags: StrapiTag[] | undefined = attrs.tags?.map((tag: RawTag) => ({
    id: tag.id,
    name: tag.name,
    slug: tag.slug,
  }));

  return {
    id: item.id,
    title: attrs.title,
    slug: attrs.slug,
    date: attrs.date,
    content: attrs.content || [],
    thumbnail,
    attachments,
    tags,
    is_featured: attrs.is_featured,
    priority: attrs.priority,
    pinned_until: attrs.pinned_until || null,
  };
}
