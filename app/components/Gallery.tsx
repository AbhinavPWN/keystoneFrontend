"use client";
  ...
  export default function Gallery() {
    const CMS_URL = process.env.NEXT_PUBLIC_CMS_URL || "http://135.181.66.188:8080";
    ...
    const fetchPage = useCallback(async (page: number) => {
      ...
      try {
        const res = await fetch(`/api/galleries?pagination[page]=${page}&pagination[pageSize]=${pageSize}&populate=Image`, {
          cache: "no-store",
        });
        ...
        const newItems: GalleryItem[] = json.data.map((item) => {
          ...
          const fullImageUrl = item.Image?.url || "/default-image.png";
          ...
          return {
            ...
            image: imageUrl.startsWith("/") ? `${CMS_URL}${imageUrl}` : imageUrl,
            fullImage: fullImageUrl.startsWith("/") ? `${CMS_URL}${fullImageUrl}` : fullImageUrl,
          };
        });
